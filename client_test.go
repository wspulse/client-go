...
func TestClient_WithHeartbeat_InvalidParams_Panics(t *testing.T) {
	cases := []struct {
		...
		{"ping == pong", 10 * time.Second, 10 * time.Second, 5 * time.Second, "wspulse: heartbeat.pingPeriod must be strictly less than pongWait"},
		{"ping > pong", 30 * time.Second, 10 * time.Second, 5 * time.Second, "wspulse: heartbeat.pingPeriod must be strictly less than pongWait"},
